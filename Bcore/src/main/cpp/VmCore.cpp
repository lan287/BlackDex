...
// Enhanced for modern packers support.
//
...
void init(JNIEnv *env, jobject clazz, jint api_level) {
    ...
    VMEnv.loadEmptyDex = env->GetStaticMethodID(VMEnv.VMCoreClass, "loadEmptyDex", "()[J");
...
// ---- New: Deep dump entry point ----

void enableDeepDump(JNIEnv *env, jclass clazz, jstring dir) {
    DexDump::enableDeepDump(env, dir);
}

// ---- New: Add hidden library path for SoHider ----

void addHiddenLibrary(JNIEnv *env, jclass clazz, jstring basename) {
    const char *name = env->GetStringUTFChars(basename, nullptr);
    if (name) {
        SoHider::addHiddenName(name);
        env->ReleaseStringUTFChars(basename, name);
    }
}

void addHiddenPath(JNIEnv *env, jclass clazz, jstring path) {
    const char *p = env->GetStringUTFChars(path, nullptr);
    if (p) {
        SoHider::addHiddenPath(p);
        env->ReleaseStringUTFChars(path, p);
    }
}

// ---- New: Anti-debug controls ----

void setAntiTraceEnabled(JNIEnv *env, jclass clazz, jboolean enabled) {
...
void installAntiDebug(JNIEnv *env, jclass clazz) {
    AntiDebug::install(env);
}
...
static JNINativeMethod gMethods[] = {
        ...
        {"addHiddenPath",        "(Ljava/lang/String;)V",                   (void *) addHiddenPath},
        {"enableDeepDump",       "(Ljava/lang/String;)V",                   (void *) enableDeepDump},
        {"installAntiDebug",     "()V",                                     (void *) installAntiDebug},
};
